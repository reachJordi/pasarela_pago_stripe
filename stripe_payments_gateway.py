...
def create_payment_method() -> str:
    ...
    try:
        ...
        #print(f"Método de pago creado: {payment_method}")
...
# Crear un pago
def create_payment(client_id: str, payment_method_id: str, product_id: str, amount: int, currency: str) -> str:
...
def add_payment_method_to_user(user_id: str, payment_method_id: str):
    try:
        ...
        print(f"Método de pago {payment_method_id} asociado a cliente {user_id}")
    except stripe.error.StripeError as e:
        print(f"Error en Stripe: {e.user_message}")
...
def get_product_price(product_id: str):
    price = stripe.Product.list(product=product_id, limit=1)
